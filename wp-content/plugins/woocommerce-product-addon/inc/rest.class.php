...
class PPOM_Rest {
    ...
    // Check if api is enable and secreky is set
    function api_enable() {
        
        $api_enable = get_option( 'ppom_api_enable', true );
    ...
    function get_ppom_meta_info_order( WP_REST_Request $request ) {
        ...
        $item_product_meta = $this->get_order_item_meta($order);
                ...
                foreach($order_item_meta as $order_product_id => $item_meta) {
                    ...
                    if( $order_product_id == $product_id ) {
                        ...
                        foreach($item_meta as $meta_key => $meta_val) {
                            ...
                            $meta_update_res = wc_update_order_item_meta($item_id, $meta_key, $meta_val);
                        }
                    }
                }
            }
    ...
    function get_order_item_meta( $order ) {
        ...
        foreach( $order->get_items() as $item_id => $item_product ){
            ...
            foreach($item_product->get_meta_data() as $meta_data) {
                ...
                $field_type = isset($fields_info['type']) ? $fields_info['type'] : '';
                
                $temp_array['id'] = $meta_data->id;
                $temp_array['key'] = $meta_data->key;
                $temp_array['value'] = $meta_data->value;
                
                if( in_array($field_type, array('radio','checkbox','select')) ) {
                
                    $temp_options = array();
                    foreach($fields_info['options'] as $option) {
                        
                        $value_in_array = explode(',', $meta_data->value);
                        
                        $option_value = stripslashes($option['option']);
                        
                        if(  in_array($option_value, $value_in_array) ) {
                            $temp_options[] = $option;
                        }
                    }
                    
                    $temp_array['value'] = $temp_options;
                }
                
                $meta_info[] = $temp_array;
