...
function ppom_get_croppie_options( $settings ) {
	...
	$viewport = !empty($settings['viewport']) ? $settings['viewport'] : '200,200';
	$viewport = explode(",", $viewport);
	
	$viewport_type = !empty($settings['viewport_type']) ? $settings['viewport_type'] : 'square';
	
	$viewport_h = $viewport[0];
	$viewport_w = $viewport[1];
	
	if( $viewport_type == 'circle' ) { // If circle then set same heigt and width
		$viewport_w = $viewport_h;
	}
	// view height, width and type
	$cropping_settigs['viewport'] = array('height'=>$viewport_h, 'width'=>$viewport_w,'type'=>$viewport_type);
	
	$boundary = !empty($settings['boundary']) ? $settings['boundary'] : '300,300';
